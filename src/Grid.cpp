...
float Grid::calculateBoundedBoxVolume() {
	float Volume = 0;
	float max_value_x = -10;
	float min_value_x = 10;
	float max_value_y = -10;
	float min_value_y = 10;
	float max_value_z = -10;
	float min_value_z = 10;
	...
	for (int i = 0; i < numCells(); i++) {
		...
		//cout << Point3d(p0).x;
		if (max_value_x < Point3d(p0).x)
		{
			max_value_x = Point3d(p0).x;
		}
		if (min_value_x > Point3d(p0).x)
		{
			min_value_x = Point3d(p0).x;
		}
		if (max_value_y < Point3d(p0).y)
		{
			max_value_y = Point3d(p0).y;
		}
		if (min_value_y > Point3d(p0).y)
		{
			min_value_y = Point3d(p0).y;
		}
		if (max_value_z < Point3d(p0).z)
		{
			max_value_z = Point3d(p0).z;
		}
		if (min_value_z > Point3d(p0).z)
		{
			min_value_z = Point3d(p0).z;
		}
